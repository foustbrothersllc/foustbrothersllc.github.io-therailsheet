  ...
  function updateRow(rowIndex: number, patch: Partial<ParsedTrailerRow>) {
    setRows((current) =>
      current.map((r) => {
        ...
        if (!updated.origin) stillMissing.push("Missing origin");
        if (!updated.destination) stillMissing.push("Missing destination");
        if (!updated.sort_type) stillMissing.push("Missing sort type");
